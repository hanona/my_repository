...
$.widgets.Carousel = function(wargs) {
    ...
    this.addItem = function(item) {
        ...
        var id = item.id;
        var text = _widget.applyTemplate(item, template);//"+text+"
        $("<div id='"+wargs.uuid + "_item_" + item.id+"'></div>").css("zIndex", 1).width(itemWidth).height(itemHeight).css('display', 'inline').css("float", "left").appendTo(_widget.container)        
